...
/*   Updated: 2018/06/11 15:15:05 by kyork            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			debug_segment_32(void *d, const void *body, size_t cmd_size)
{
	...
	data = d;
	cmd = body;
	ft_printf("Load command LC_SEGMENT: %zd bytes\n", cmd_size);
	ft_printf("  segment name: %.16s\n", cmd->segname);
	ft_printf("  vmaddr: %#llx\tvmsize: %lld\n", swap64i(cmd->vmaddr),
			swap64i(cmd->vmsize));
	ft_printf("  fileoff: %lld\tfilesize: %lld\t", swap64i(cmd->fileoff),
...
void			debug_segment_64(void *d, const void *body, size_t cmd_size)
{
	...
	data = d;
	cmd = body;
	ft_printf("Load command LC_SEGMENT_64: %zd bytes\n", cmd_size);
	ft_printf("  segment name: %.16s\n", cmd->segname);
	ft_printf("  vmaddr: %#llx\tvmsize: %lld\n", swap64i(cmd->vmaddr),
			swap64i(cmd->vmsize));
	ft_printf("  fileoff: %lld\tfilesize: %lld\t", swap64i(cmd->fileoff),
